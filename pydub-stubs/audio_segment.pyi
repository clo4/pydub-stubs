...
class AudioSegment:
    ...
    # The following functions are defined by effects.py, but they are "registered" to the AudioSegment
    # class as methods. This is done by default, whereas the effects defined by scipy_effects.py must be
    # manually imported. Since these will always be available on the class, even though some may be overridden,
    # it's worth it to treat them as methods defined on the class itself, since that's the user experience.
    def apply_mono_filter_to_each_channel(self, filter_fn: Callable[[Self], Self]) -> Self: ...
    def normalize(self, headroom: float = ...) -> Self: ...
    def speedup(self, playback_speed: float = ..., chunk_size: int = ..., crossfade: int = ...) -> Self: ...
    def strip_silence(self, silence_len: int = ..., silence_thresh: int = ..., padding: int = ...) -> Self: ...
    def compress_dynamic_range(
        self, threshold: float = ..., ratio: float = ..., attack: float = ..., release: float = ...
    ) -> Self: ...
    def invert_phase(
        self,
        channels: tuple[Literal[1], Literal[1]] | tuple[Literal[1], Literal[0]] | tuple[Literal[0], Literal[1]] = ...,
    ) -> Self: ...
    def low_pass_filter(self, cutoff: float) -> Self: ...
    def high_pass_filter(self, cutoff: float) -> Self: ...
    def pan(self, pan_amount: float) -> Self: ...
    def apply_gain_stereo(self, left_gain: float = ..., right_gain: float = ...) -> Self: ...
